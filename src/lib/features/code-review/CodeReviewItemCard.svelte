<script lang="ts">
	...
	import type { CodeReviewListEntry } from '$lib/types';
	import {
		commentAuthorLabel,
	...
	const app = getApp();
	const row = $derived(getCodeReviewObservationRow(entry.categoryId, entry.observationId));
	const owner = $derived(entry.assignee);
	...
	const flagged = $derived(row[owner] === 'decline');
...
>
	<div class="flex items-start gap-2 px-3 py-2.5">
		<div class="min-w-0 flex-1">
			<div class="flex flex-wrap items-center gap-2">
				<span class="font-mono text-[10px] uppercase tracking-wide text-kood-muted">{entry.compositeId}</span>
				{#if flagged}
	...
	>
		...
		{#if canSetVerdict}
			<div class="ml-auto flex flex-wrap gap-1">
				<button
					type="button"
					class="rounded px-2 py-0.5 text-[11px] font-medium {row[self!] === 'decline'
					...
					onclick={() =>
						setCodeReviewVerdict(entry.categoryId, entry.observationId, self!, 'decline')}>Decline</button
				>
				<button
					type="button"
					class="rounded px-2 py-0.5 text-[11px] font-medium {row[self!] === 'accept'
	...
	{#if open}
		<div class="space-y-3 border-t border-kood-border px-3 pb-3 pt-2.5">
			...
			{#if isReviewer && self}
				<div>
					<label class="text-[11px] text-kood-muted" for="cr-d-{idSafe}">Comment</label>
			...
			{#if isSandra}
				<div>
					<label class="text-[11px] text-kood-muted" for="cr-s-{idSafe}">Reply</label>
					<div class="relative mt-1">
						<textarea
							id="cr-s-{idSafe}"
							rows="2"
			...
			{#if row.verdictHistory.length > 0}
				<details class="text-xs text-kood-muted">
					<summary class="cursor-pointer text-kood-text/70">Past rounds ({row.verdictHistory.length})</summary>
