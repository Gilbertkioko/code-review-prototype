...
<div class="mx-auto max-w-3xl px-4 py-10 text-kood-text">
	<p class="text-xs text-kood-muted">
		<a href="/admin" class="text-kood-accent underline">← Admin</a>
	</p>
	<h1 class="mt-4 font-mono text-xl font-semibold">Project audit</h1>
	<p class="mt-2 text-sm text-kood-muted">Status: <span class="text-kood-accent">{data.project.status}</span></p>
	...
	{#if data.testingItemProgress.length > 0}
		<section class="mt-8 overflow-x-auto">
			...
			<p class="mt-1 text-xs text-kood-muted">Latest accept/decline/pending per checklist row (same ids as the prototype).</p>
			<table class="mt-3 w-full min-w-[640px] border-collapse text-left text-xs">
				<thead>
					<tr class="border-b border-kood-border text-kood-muted">
						<th class="py-2 pr-2">Item</th>
						<th class="py-2 pr-2">Section</th>
						<th class="py-2 pr-2">Owner</th>
						<th class="py-2 pr-2">Reviewer 1</th>
						<th class="py-2 pr-2">Reviewer 2</th>
				...
				<tbody>
					{#each data.testingItemProgress as row (row.itemId)}
						<tr class="border-b border-kood-border/50">
							<td class="py-2 pr-2 font-mono text-[11px]">{row.itemId}</td>
							<td class="py-2 pr-2">{row.section}</td>
							<td class="py-2 pr-2">{row.mandatoryOwner ?? '—'}</td>
							<td class="py-2 pr-2">{row.janeVerdict}</td>
							<td class="py-2 pr-2">{row.joeVerdict}</td>
							<td class="py-2 pr-2">{row.testingRound}</td>
						</tr>
					{/each}
				</tbody>
			</table>
	...
	{#if data.codeReviewObservationProgress.length > 0}
		<section class="mt-8 overflow-x-auto">
			...
			<table class="mt-3 w-full min-w-[560px] border-collapse text-left text-xs">
				<thead>
					<tr class="border-b border-kood-border text-kood-muted">
						<th class="py-2 pr-2">Category</th>
						<th class="py-2 pr-2">Observation</th>
						<th class="py-2 pr-2">Reviewer 1</th>
						<th class="py-2 pr-2">Reviewer 2</th>
						<th class="py-2 pr-2">Round</th>
					</tr>
				...
				<tbody>
					{#each data.codeReviewObservationProgress as row (`${row.categoryId}-${row.observationId}`)}
						<tr class="border-b border-kood-border/50">
							<td class="py-2 pr-2">{row.categoryId}</td>
							<td class="py-2 pr-2 font-mono text-[11px]">{row.observationId}</td>
							<td class="py-2 pr-2">{row.janeVerdict}</td>
							<td class="py-2 pr-2">{row.joeVerdict}</td>
							<td class="py-2 pr-2">{row.codeReviewRound}</td>
						</tr>
					{/each}
				</tbody>
			</table>
	...
	<section class="mt-8">
		...
		<ul class="mt-3 space-y-4 text-sm">
			{#each data.testingThreads as t, ti (ti)}
				<li class="rounded-lg border border-kood-border bg-kood-surface/60 p-3">
	...
	{#if data.project.testingJson || data.project.codeReviewJson}
		<section class="mt-8">
			...
			{#if data.project.testingJson}
				...
				<pre
					class="mt-1 max-h-48 overflow-auto rounded border border-kood-border bg-kood-bg p-3 text-xs text-kood-muted">{data.project.testingJson}</pre>
			{/if}
			{#if data.project.codeReviewJson}
				<p class="mt-3 text-xs text-kood-muted">code_review_json</p>
				<pre
					class="mt-1 max-h-48 overflow-auto rounded border border-kood-border bg-kood-bg p-3 text-xs text-kood-muted">{data.project.codeReviewJson}</pre>
			{/if}
		</section>
	{/if}
</div>
